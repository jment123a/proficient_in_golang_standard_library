...
// 如果ns < 0，则获取m的信号量并返回0。
// 如果ns >= 0，请尝试获取最多ns纳秒的m信号量。
// 如果获取了信号量，则返回0；如果被中断或超时，则返回-1。
//
// func semawakeup(mp *m)
// 唤醒mp，它已经或即将在其信号灯上处于休眠状态。
//
const (
	locked uintptr = 1 // 注：已锁状态
...
func lock(l *mutex) {
	gp := getg()        // 注：获取当前g
	if gp.m.locks < 0 { // 注：如果当前m不可用，抛出异常
		throw("runtime·lock: lock count") // 注：抛出异常，锁数量
	}
	gp.m.locks++ // 注：信号量计数+1

	// 投机抢锁。
	if atomic.Casuintptr(&l.key, 0, locked) { // 注：原子性地设置l的锁
		return
	}
	semacreate(gp.m)
