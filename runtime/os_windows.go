...
type mOS struct {
	...
	waitsema   uintptr // 停车位信号灯
	resumesema uintptr // 指示暂停/恢复的信号灯
...
// 使用stdcall约定调用Windows函数，并在调用期间切换到os堆栈。
...
//go:nowritebarrier
//go:nosplit
func stdcall(fn stdFunction) uintptr { // 注：#
	gp := getg()                                // 注：获取当前g
	mp := gp.m                                  // 注：获取当前g的m
	mp.libcall.fn = uintptr(unsafe.Pointer(fn)) // 注：方法为fn
	resetLibcall := false
	if mp.profilehz != 0 && mp.libcallsp == 0 { // 注：#如果#和堆栈指针寄存器为空
		// 将PC/SP留给CPU Profiler
		mp.libcallg.set(gp)          // 注：将m的g设置为当前g
		mp.libcallpc = getcallerpc() // 注：获取调用者的调用者的程序计数器
		// sp必须是最后一个，因为一旦异步cpu探查器发现所有三个值都不为零，它将使用它们
		mp.libcallsp = getcallersp() // 注：获取调用者的调用者的堆栈指针
		resetLibcall = true          // 查看sys_darwin.go：libcCall中的评论
	}
	asmcgocall(asmstdcallAddr, unsafe.Pointer(&mp.libcall)) // 注：#
	if resetLibcall {                                       // 注：#
		mp.libcallsp = 0 // 注：堆栈指针归零
	}
	return mp.libcall.r1 // 注：获取返回值
}
...
func stdcall5(fn stdFunction, a0, a1, a2, a3, a4 uintptr) uintptr {
	mp := getg().m
	mp.libcall.n = 5                                         // 注：参数数量为5
	mp.libcall.args = uintptr(noescape(unsafe.Pointer(&a0))) // 注：参数为栈中的a0-a5
	return stdcall(fn)
}
