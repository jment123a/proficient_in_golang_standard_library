...
const (
	uintptrMask = 1<<(8*sys.PtrSize) - 1 // 注：63个1

	// Goroutine抢占请求。
	// 存储到g -> stackguard0中会导致拆分堆栈检查失败。
	// 必须大于任何实际sp。
	// 0xfffffade以十六进制表示。
	stackPreempt = uintptrMask & -1314 // 注：1 ... 10100100010

	// 线程正在派生。
	// 存储到g -> stackguard0中会导致拆分堆栈检查失败。
	// 必须大于任何实际sp。
	stackFork = uintptrMask & -1234 // 注：1 ... 10011010010
)
