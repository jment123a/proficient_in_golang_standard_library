...
func stopTheWorldWithSema() {
	...
	// 如果我们持有一个锁，那么我们将无法停止试图获取该锁的另一个m。
	if _g_.m.locks > 0 {
		throw("stopTheWorld: holding locks") // 恐慌："持有锁"
	}
...
// newmHandoff 包含需要新的OS线程的m结构的列表。
// 在newm本身无法安全启动OS线程的情况下，newm使用它。
var newmHandoff struct {
	lock mutex

	// newm 指向需要新OS线程的M结构的列表。 该列表通过m.schedlink链接。
	newm muintptr
	...
	// haveTemplateThread 表示templateThread已启动。 这不受锁保护。 使用cas设置为1。
...
// startTemplateThread启动模板线程（如果尚未运行）。
//
// 调用线程本身必须处于已知良好状态。
func startTemplateThread() {
	if GOARCH == "wasm" { // wasm上尚无线程
		return
	}

	// 禁用抢占功能，以确保一旦设置haveTemplateThread，就将在停放之前创建模板线程。
	mp := acquirem()                                        // 注：#将当前g与m设置锁
	if !atomic.Cas(&newmHandoff.haveTemplateThread, 0, 1) { // 注：#
		releasem(mp)
		return
	}
...
// 直到调用goroutine对UnlockOSThread和LockOSThread的调用次数相同。
// 如果调用的goroutine在没有解锁线程的情况下退出，则该线程将终止。
//
// 所有init函数都在启动线程上运行。 从init函数调用LockOSThread将导致在该线程上调用main函数。
//
// goroutine应该在调用依赖于每个线程状态的OS服务或非Go库函数之前调用LockOSThread。
func LockOSThread() {
	if atomic.Load(&newmHandoff.haveTemplateThread) == 0 && GOOS != "plan9" {
		// 如果需要从锁定线程中启动新线程，则需要模板线程。 在我们处于良好状态时，请立即启动。
		startTemplateThread()
	}
	_g_ := getg()
	_g_.m.lockedExt++
