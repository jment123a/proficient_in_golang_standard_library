...
// 应该是不安全的内置指针吗？
//go:nosplit
func add(p unsafe.Pointer, x uintptr) unsafe.Pointer { // 注：返回p偏移x后的指针
	return unsafe.Pointer(uintptr(p) + x)
}

// getg 返回当前g的指针。
// 编译器将此函数的调用重写为直接获取g的指令（从TLS或专用寄存器）。
func getg() *g // 注：h获取当前g
...
// getcallersp 返回其调用者的调用者的堆栈指针（SP）。
// 实现可能是编译器固有的； 不一定在每个平台上都实现此代码。
//
// 例如：
//
...
//
// 对getcallerpc和getcallersp的调用必须在所询问的框架中完成。
//
// getcallersp的结果在返回时是正确的，但是随后的任何对函数的调用都可能使该函数无效，该函数可能会重新定位堆栈以增大或缩小堆栈。
// 一般规则是，应立即使用getcallersp的结果，并且只能将其传递给nosplit函数。

//go:noescape
func getcallerpc() uintptr // 注：获取调用者的调用者的程序计数器（PC）

//go:noescape
func getcallersp() uintptr // 在所有平台上均作为内在实现，注：获取调用者的调用者的堆栈指针（SP）
