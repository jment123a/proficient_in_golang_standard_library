...
// 将KeepAlive标记为noinline，以便将其轻松检测为内在函数。
//go:noinline

// KeepAlive 将其参数标记为当前可达。
// 这样可以确保在程序中调用KeepAlive的点之前，不释放对象，并且不运行其终结器。
//
// 一个非常简化的示例，显示需要KeepAlive的位置：
...
// 	// 此后不再使用p。
//
// 如果没有KeepAlive调用，则终结器可以在syscall.Read的开头运行，在syscall.Read进行实际的系统调用之前关闭文件描述符。
func KeepAlive(x interface{}) {
	// 采用编译器无法消除的x用法。
	// 确保x在输入时仍然有效。 我们需要x才能在"defer runtime.KeepAlive(x)"条目中保持活动状态； 请参阅问题21402。
	if cgoAlwaysFalse {
		println(x)
	}
}
