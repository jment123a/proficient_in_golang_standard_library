// 通过在golang.org/x/text中运行"go generate"生成的代码。 请勿编辑。

package unicode

// Version 是从中派生表的Unicode版本。
...
// 注：不同类型的字符集
var (
	Cc     = _Cc // (Other, control)			Cc是类别Cc（其他，控件）中的Unicode字符集。
	Cf     = _Cf // (Other, format)				Cf是类别Cf（其他，格式）中的Unicode字符集。
	Co     = _Co // (Other, private use)		Co是Co类别（其他，供私人使用）中的Unicode字符集。
	Cs     = _Cs // (Other, surrogate)			Cs是类别Cs（其他，替代）中的Unicode字符集。
	Digit  = _Nd // (Number, decimal digit)		Digit是具有"十进制数字"属性的Unicode字符集。
	Nd     = _Nd // (Number, decimal digit)		Nd是类别Nd（数字，十进制数字）中的Unicode字符集。
	Letter = _L  // (Letter)					Letter/L是类别L的Unicode字母的集合。
	L      = _L  // (Letter)
	Lm     = _Lm // (Letter, modifier)			Lm是类别Lm（字母，修饰符）中的Unicode字符集。
	Lo     = _Lo // (Letter, other)				Lo是Lo类别（字母，其他）中的Unicode字符集。
	Lower  = _Ll // (Letter, lowercase)			小写是Unicode小写字母的集合。
	Ll     = _Ll // (Letter, lowercase)			L1是类别L1（字母，小写）中的Unicode字符集。
	Mark   = _M  // (Mark)						Mark/M是Unicode标记字符集，类别M。
	M      = _M  // (Mark)
	Mc     = _Mc // (Mark, spacing combining)	Mc是Mc类别（标记，间隔组合）中的Unicode字符集。
	Me     = _Me // (Mark, enclosing)			Me是类别Me（标记，包含）中的Unicode字符集。
	Mn     = _Mn // (Mark, nonspacing)			Mn是Mn类别（标记，非间距）中的Unicode字符集。
	Nl     = _Nl // (Number, letter)			N1是类别N1（数字，字母）中的Unicode字符集。
	No     = _No // (Number, other)				No是类别No（数字，其他）中的Unicode字符集。
	Number = _N  // (Number)					Number/N是Unicode数字字符集，类别N。
	N      = _N  // (Number)
	Other  = _C  // (Other)						Other/C是Unicode控件和特殊字符的集合，类别C。
	C      = _C  // (Other)
	Pc     = _Pc // (Punctuation, connector)	Pc是类别Pc（标点，连接器）中的Unicode字符集。
	Pd     = _Pd // (Punctuation, dash)			Pd是类别Pd（标点，破折号）中的Unicode字符集。
	Pe     = _Pe // (Punctuation, close)		Pe是Pe类别（标点，关闭）中的Unicode字符集。
	Pf     = _Pf // (Punctuation, final quote)		Pf是类别Pf（标点，最后引号）中的Unicode字符集。
	Pi     = _Pi // (Punctuation, initial quote)	Pi是类别Pi（标点，初始引号）中的Unicode字符集。
	Po     = _Po // (Punctuation, o)				Po是类别Po c中的Unicode字符集。
	Ps     = _Ps // (Punctuation, open)				Ps是类别Ps（标点，打开）中的Unicode字符集。
	Punct  = _P  // (Punctuation)					Punct/P是Unicode标点字符集，类别P。
	P      = _P  // (Punctuation)
	Sc     = _Sc // (Symbol, currency)			Sc是类别Sc（符号，货币）中的Unicode字符集。
	Sk     = _Sk // (Symbol, modifier)			Sk是类别Sk（符号，修饰符）中的Unicode字符集。
	Sm     = _Sm // (Symbol, math)				Sm是类别Sm（符号，数学）中的Unicode字符集。
	So     = _So // (Symbol, other)				So类别中的Unicode字符集也是如此（Symbol，其他）
	Space  = _Z  // (Space)						Space/Z是Unicode空格字符集，类别Z。
	Z      = _Z  // (Space)
	Symbol = _S  // (Symbol)					Symbol/S是Unicode符号字符集，类别S。
	S      = _S  // (Symbol)
	Title  = _Lt // (Title)						Title是Unicode标题大小写字母的集合。
	Lt     = _Lt // (Letter, titlecase)			Lt是类别Lt（字母，标题大小写）中的Unicode字符集。
	Upper  = _Lu // (Letter, uppercase)			Upper是Unicode大写字母的集合。
	Lu     = _Lu // (Letter, uppercase)			Lu是Lu类别（字母，大写）中的Unicode字符集。
	Zl     = _Zl // (Separator, line)			Zl是类别Zl（分隔符，行）中的Unicode字符集。
	Zp     = _Zp // (Separator, paragraph)		Zp是类别Zp（分隔符，段落）中的Unicode字符集。
	Zs     = _Zs // (Separator, space)			Zs是类别Zs（分隔符，空格）中的Unicode字符集。
)
...
// 这些变量的类型为*RangeTable。
var (
	Adlam                  = _Adlam                  // Adlam是脚本Adlam中的Unicode字符集。
...
// CaseRanges 是描述具有非自我映射的所有字母的大小写映射的表。
...
var properties = [MaxLatin1 + 1]uint8{ // 注：小于Latin1的字符与对应类型，快速路经
	0x00: pC,       // '\x00'
	0x01: pC,       // '\x01'
	0x02: pC,       // '\x02'
	0x03: pC,       // '\x03'
...
// FoldCategory 将类别名称映射到类别外部的代码点表，这些代码点在简单的大小写折叠下等同于类别内部的代码点。
// 如果没有类别名称的条目，则没有这些点。
...
// FoldScript 将脚本名称映射到脚本外部的代码点表，该表在简单的大小写折叠下等同于脚本内部的代码点。
// 如果没有脚本名称的条目，则没有这些点。
...
// 范围条目：3483 16位，1730 32位，共5213。
// 范围字节：20898 16位，20760 32位，总计41658。
